...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    fc2_weights = tf.Variable(tf.truncated_normal([2, NUM_LABELS], stddev=0.1, seed=SEED, dtype=data_type()))
    ...
    xent_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits, train_labels_node))
    beta = 1e-3
    ...
    def calculate_embeddings(data, sess):
        ...
        for begin in xrange(0, size, EVAL_BATCH_SIZE):
            ...
            if end <= size:
                predictions[begin:end, :] = sess.run(eval_embeddings, feed_dict={eval_data: data[begin:end, ...]})
            else:
    ...
    with tf.Session() as sess:
        ...
        for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
            ...
            if step % EVAL_FREQUENCY == 0:
                ...
                print(
                    'Validation error: %.1f%%' % error_rate(eval_in_batches(validation_data, sess), validation_labels))
                sys.stdout.flush()
